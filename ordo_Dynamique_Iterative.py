...
pi = pd.read_csv("LesDonnesMeta50.csv",delimiter="\t")['Pi'].values
wi = pd.read_csv("LesDonnesMeta50.csv",delimiter="\t")['Wi'].values
di = pd.read_csv("LesDonnesMeta50.csv",delimiter="\t")['Di'].values
n = len(pi)
...
for i in range(3,len(k)+1):
    ...
    for t in range(len(combinaison1)):
        ...
        for h in range(len(combinaison1[t])):
            ...
            resultP[str(etapeII[r][1]) +' => ' + str(combinaison1[t][h])]= etapeII[r][0] + wi[combinaison1[t][h] - 1] * max(0, (ci - (di[combinaison1[t][h] - 1])))
        mino=min(resultP.values())
        n = min(resultP, key=resultP. get)
        #n=resultP.values().index(mino)
        v = resultP[n]
